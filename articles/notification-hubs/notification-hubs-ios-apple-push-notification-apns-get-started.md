---
title: "Odesílání nabízených oznámení do systému iOS pomocí Azure Notification Hubs | Dokumentace Microsoftu"
...
4. Stáhněte si [Mobile Services iOS SDK verze 1.2.4] a soubor rozbalte. V Xcode klikněte pravým tlačítkem na projekt a klikněte na možnost **Přidat soubory do** a přidejte složku **WindowsAzureMessaging.framework** do projektu Xcode. Vyberte možnost **Kopírovat položky v případě potřeby** a pak klikněte na tlačítko **Přidat**.
   ...
   > [!NOTE]
   > Centrum oznámení SDK aktuálně nepodporuje bitcode na Xcode 7.  Je nutné nastavit **Povolit Bitcode** na hodnotu **Ne** v části **Sestavení možnosti** pro váš projekt.
   > 
   > 
...
1. Sestavte a spusťte aplikaci na vašem zařízení a ověřte, zda nedochází k žádným chybám.
...
## <a name="optional-send-push-notifications-from-the-app"></a>(Volitelné) Zasílání nabízených oznámení z aplikace
> [!IMPORTANT]
...
1. V `ViewController.m` přidejte následující kód do implementace rozhraní k vygenerování tokenu autorizace SaS, která bude k dispozici v hlavičce **Autorizace**, jak je uvedeno v [referenci rozhraní API REST](http://msdn.microsoft.com/library/azure/dn495627.aspx).
...
2. Využijte zkratku CTRL + přetažení z tlačítka **Odeslat oznámení** do `ViewController.m` a přidejte akci s názvem **SendNotificationMessage** pro událost **Touch Down**. Metoda aktualizace pomocí následujícího kódu pro odeslání oznámení pomocí rozhraní REST API.
...
3. V `ViewController.m` přidejte následující metodu delegáta pro podporu zavření klávesnice pro textové pole. Využijte zkratku CTRL + přetažení z textového pole na ikonu řadiče zobrazení v návrháři rozhraní pro nastavení řadiče zobrazení jako delegáta výstupu.
...
4. V `ViewController.m` přidejte následující metody delegáta pro podporu analýzy odpovědi pomocí `NSXMLParser`.
...
5. Sestavte projekt a ověřte, že se zde nenachází žádné chyby.

> [!NOTE]
> Pokud dojde k chybě sestavení v Xcode7 o podpoře bitcode, měli byste změnit **Nastavení sestavení** > **Povolit Bitcode (ENABLE_BITCODE)** na **NE** v Xcode. Centra oznámení SDK aktuálně nepodporují bitcode. 
> 
> 
