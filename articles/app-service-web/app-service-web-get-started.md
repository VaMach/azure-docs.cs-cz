<properties 
    pageTitle="Nasazení první webové aplikace do Azure během 5 minut" 
    description="Zjistěte, jak snadno lze spustit webové aplikace v službě App Service, prostřednictvím nasazení ukázkové aplikace v pouhých několika krocích. Začněte provádět skutečný vývoj během 5 minut a ihned sledujte výsledky." 
...
# Nasazení první webové aplikace do Azure během 5 minut

[AZURE.INCLUDE [tabs](../../includes/app-service-web-get-started-nav-tabs.md)]

Tento kurz vám pomůže nasadit první webovou aplikaci do služby [Azure App Service](../app-service/app-service-value-prop-what-is.md). Služba App Service vám umožňuje vytvářet webové aplikace [back-endy mobilních aplikací](/documentation/learning-paths/appservice-mobileapps/) a [aplikace API](../app-service-api/app-service-api-apps-why-best-platform.md).
...
1. `CD` do pracovního adresáře a naklonujte ukázkovou aplikaci takto:
...
2. `CD` do úložiště ukázkové aplikace. Například: 
...
4. Následujícím příkazem vytvořte v Azure prostředek aplikace služby App Service s jedinečným názvem aplikace. Po zobrazení výzvy zadejte číslo požadované oblasti.

        azure site create --git <app_name>
    
    ![Vytvoření prostředku Azure pro první webovou aplikaci v Azure](./media/app-service-web-get-started/4-create-site.png)
    
    >[AZURE.NOTE] Pokud jste u předplatného Azure dosud nenastavili přihlašovací údaje nasazení, budete vyzváni k jejich vytvoření. Tyto přihlašovací údaje (tedy nikoli přihlašovací údaje účtu Azure) jsou službou App Service používány pouze k nasazením Git a přihlášením FTP. 
    ...
    Můžete procházením zobrazit adresu URL aplikace (http://&lt;app_name>.azurewebsites.net) a prohlédnout si krásnou výchozí stránku HTML, ale nyní tam raději umístíme váš vlastní kód.
...
Nyní můžete pomocí Git kdykoli provádět nucené doručení (push) z kořenového adresáře projektu (úložiště) a aktualizovat živý web. Postup je stejný jako při prvním nasazení aplikace do Azure. Například pokaždé, když chcete nuceně doručit (push) novou změnu, kterou jste místně otestovali, stačí spustit následující příkazy z kořenového adresáře projektu (úložiště):
    
    git add .
    git commit -m "<your_message>"
    git push azure master

## Zobrazení aplikace na portálu Azure

Nyní přejdeme na portál Azure a prohlédneme si, co jste vytvořili:

1. Přihlaste se k [portálu Azure](https://portal.azure.com) pomocí účtu Microsoft, který obsahuje předplatné Azure.
...
Okno portálu aplikace služby App Service nabízí bohatou sadu nastavení a nástrojů, které můžete využít ke konfiguraci, monitorování, zabezpečení či řešení problémů aplikace. Seznamte se s tímto rozhraním tím, že provedete několik jednoduchých úloh (číslo úlohy odpovídá číslu na snímku obrazovky):
...
- Zdokonalte aplikaci Azure o další úroveň. Zabezpečte ji pomocí ověřování. Škálujte ji na základě poptávky. Nastavte některá upozornění týkající se výkonu. To vše pomocí několika kliknutí. Viz téma [Přidání funkce do první webové aplikace](app-service-web-get-started-2.md).
- Kromě Git a rozhraní příkazového řádku Azure CLI existují i další způsoby jak nasadit webové aplikace do Azure (viz téma [Nasazení aplikace do služby Azure App Service](../app-service-web/web-sites-deploy.md)).
