...
1. Na následující tlačítko použijte pro přihlášení do Azure a otevřete šablonu na portálu Azure.
    ...
    Šablona Azure Resource Manager je umístěna ve **https://hditutorialdata.blob.core.windows.net/armtemplates/create-linux-based-kafka-storm-cluster-in-vnet-v1.json**. Vytvoří v následujících zdrojích informací:
...
2. Použijte následující pokyny k naplnění položek na **vlastní nasazení** okno:
...
Po vytvoření prostředky, zobrazí se okno pro skupinu prostředků.
...
2. Jakmile se spustí topologii, otevřete uživatelské rozhraní Storm. Tato webového uživatelského rozhraní se nachází v https://storm-BASENAME.azurehdinsight.net/stormui. Nahraďte __BASENAME__ s základní název použita při vytvoření clusteru. 
