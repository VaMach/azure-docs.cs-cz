...
* Zobrazení Hive: Zobrazení Hive umožňuje spouštět dotazy Hive přímo z webového prohlížeče. Můžete uložit dotazy, zobrazení výsledků, uložte výsledky do úložiště clusteru nebo stáhnout výsledky do místního systému. Další informace o používání Hive zobrazení najdete v tématu [použijte zobrazení Hive s HDInsight](hdinsight-hadoop-use-hive-ambari-view.md).
