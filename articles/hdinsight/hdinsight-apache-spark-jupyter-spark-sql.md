...
* **Klient Secure Shell (SSH)**: systémy Linux, Unix a OS X poskytují klienta SSH pomocí příkazu `ssh`. Pro klienty Windows najdete informace v tématu [Použití SSH se systémem Hadoop ve službě HDInsight ze systému Windows pomocí klienta PuTTY](hdinsight-hadoop-linux-use-ssh-windows.md). Pro systémy Linux, Unix a OS X najdete informace v tématu [Použití SSH se systémem Hadoop ve službě HDInsight ze systému Linux, Unix nebo OS X](hdinsight-hadoop-linux-use-ssh-unix.md).
...
V tomto článku budete používat jádro PySpark. Další informace o obou jádrech najdete v tématu s popisem [použití jader poznámkových bloků Jupyter s clustery Apache Spark v HDInsight](hdinsight-apache-spark-jupyter-notebook-kernels.md). Zde je několik z klíčových výhod používání jádra PySpark:
