---
title: "Konfigurace přesměrování pro službu Azure Application Gateway – PowerShell | Dokumentace Microsoftu"
description: "Tato stránka obsahuje scénáře ke konfiguraci přesměrování pro službu Application Gateway s využitím PowerShellu."
...
Aplikační brána podporuje možnost přesměrovat provoz na základě definované konfigurace. Další obecné informace o přesměrování najdete v tématu [Přehled přesměrování ve službě Application Gateway](application-gateway-redirect-overview.md). Tento článek obsahuje příklady přesměrování HTTP na HTTPS, přesměrování na základě cest, přesměrování pro více webů a přesměrování na externí weby.

## <a name="http-to-https-redirect-on-an-existing-application-gateway"></a>Přesměrování HTTP na HTTPS v existující aplikační bráně

Následující příklad vytvoří nový naslouchací proces HTTP na portu 80 pro přesměrování požadavků do existujícího naslouchacího procesu na portu 443.
...
Jakmile se aplikační brána aktualizuje, otestujte nový naslouchací proces na portu 80.  Následující příklad ukazuje použití příkazu `curl` k otestování přesměrování.
...
Následující příklad vytvoří v existující aplikační bráně nový naslouchací proces a pravidlo na základě cest, které přesměruje pouze provoz odpovídající konkrétní cestě URL.
...
Následující příklad vytvoří novou aplikační bránu se 2 naslouchacími procesy pro více webů na portu 80. Naslouchací procesy jsou pro weby adatum.com a adatum.org. Vytvoří se pravidlo přesměrování, které bude směrovat provoz z adatum.org na adatum.com. Pro konfiguraci aliasů CNAME k veřejné IP adrese aplikační brány je vyžadována další konfigurace. Další informace o delegování domény do Azure DNS a vytváření záznamů CNAME pro doménu najdete v tématu [Delegování domény do Azure DNS](../dns/dns-delegate-domain-azure-dns.md).
...
## <a name="redirect-to-an-external-site"></a>Přesměrování na externí web

Následující příklad přesměruje provoz přicházející do aplikační brány na externí web (bing.com). Při použití přepínače `TargetUrl` nejsou povoleny přepínače `IncludePath` a `IncludeQuery`.
...
Navštivte téma [Konfigurace koncového šifrování protokolu SSL s aplikační bránou pomocí PowerShellu](application-gateway-end-to-end-ssl-powershell.md).
