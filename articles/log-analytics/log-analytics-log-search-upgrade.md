...
---
# <a name="upgrade-your-azure-log-analytics-workspace-to-new-log-search"></a>Upgrade pracovní prostor analýzy protokolů Azure na nové hledání protokolu

> [!NOTE]
> Upgrade na novou analýzy protokolů dotazovací jazyk je aktuálně volitelné poskytnutí času na [rychle pochopit práci na nový jazyk](https://go.microsoft.com/fwlink/?linkid=856078).  

Nové dotazovací jazyk analýzy protokolů je zde a je třeba upgradovat pracovního prostoru využívat výhod ho.  Tento článek popisuje výhody nový jazyk a jak převést pracovního prostoru.  Pokud si zvolíte možnost upgradovat nyní, pracovního prostoru bude nadále fungovat stejně jako vždy nebyla, ale bude automaticky převedena později.  Čas a oznámení obdrží toto datum je nastavena.

Tento článek poskytuje podrobné informace o nový jazyk a proces upgradu.
...
Upgrade bude vrácena přes všechny oblasti Azure, mohou být k dispozici v některé oblasti než jiné.  Budete vědět pracovního prostoru se dá upgradovat, je-li se zobrazí Banner informující o fialové v horní části pracovního prostoru pozvat k upgradu.
...
Při převodu pracovního prostoru, všechny uložená hledání, pravidla výstrah a zobrazení, které jste vytvořili pomocí návrháře zobrazení jsou automaticky převeden na požadovaném jazyce.  Hledání součástí řešení se nepřevádějí automaticky, ale budou se místo toho převést za chodu při jejich otevření.  Toto je zcela transparentní pro vás.

## <a name="can-i-go-back-after-i-upgrade"></a>Můžete přejít zpět po upgradu?
Při upgradu, se provede úplné zálohy pracovního prostoru, který obsahuje snímek všech uložená hledání, pravidlo výstrahy a zobrazení.  To umožňuje obnovit váš starý pracovní prostor by měl později vyžadujete-li.

Chcete-li obnovit pracovního prostoru starší verzi, přejděte na **nastavení** v pracovním prostoru a potom **upgradu Souhrn**.  Pak můžete vybrat možnost **obnovit starší verze prostoru**.  

![Obnovit starší verze](media/log-analytics-log-search-upgrade/restore-legacy-b.png)

## <a name="how-do-i-perform-the-upgrade"></a>Jak se provádí upgrade?
Pracovní prostor můžete upgradovat až uvidíte fialové hlavičky v horní části portálu.  

1.  Zahájit proces upgradu kliknutím na fialové informační zpráva, která uvádí, že **Další informace a upgradujte**.<br>![Upgrade 2](media/log-analytics-log-search-upgrade/upgrade-01a.png)<br>
2.  Si přečíst další informace o upgradu na stránce informace o upgradu.<br>![Upgrade 2](media/log-analytics-log-search-upgrade/upgrade-03.png)<br>
3.  Klikněte na tlačítko **upgradovat nyní** spusťte upgrade.<br>![Upgrade 4](media/log-analytics-log-search-upgrade/upgrade-04.png)<br>Pole oznámení v pravém horním rohu zobrazuje stav.<br>![Upgrade 5](media/log-analytics-log-search-upgrade/upgrade-05.png)
4.  A to je vše!  Přejděte na stránku hledání protokolů a podívejte se na pracovní prostor upgradovaný.<br>![Upgrade 6](media/log-analytics-log-search-upgrade/upgrade-06.png)<br>
...
Pokud jste již obeznámeni s starší verze dotazovacího jazyka pro analýzy protokolů, když, můžete tento převaděč jazyk, který se přidal do pracovního prostoru jako součást upgradu.

Jednoduše zadejte starší verze dotaz a pak klikněte na tlačítko **převést** zobrazíte přeložená verze.  Můžete buď klikněte na tlačítko Hledat spuštění vyhledávání nebo kopírování a vložení převedený dotaz, použít někde jinde, například pravidlo výstrahy.
