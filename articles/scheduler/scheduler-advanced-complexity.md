...
|:--- |:--- |:--- |:--- |:--- |:--- |
| ***čas spuštění*** |Řetězec |Ne |Žádný |ISO 8601 hodnoty data a času |<code>"startTime" : "2013-01-09T09:30:00-08:00"</code> |
...
Za těchto podmínek *první spuštění* bude 2015-04-09 v 14:00\. Modul Scheduler vypočítá výskytů provádění z čas spuštění.  Všechny instance v minulosti se zahodí. Modul používá další instance, ke kterému dochází v budoucnu.  Proto v tomto případě *startTime* je 2015-04-07 na 2:00 pm, další instance je dvou dnů od té doby je 2015-04-09 na 2:00 pm.
...
| **dny v týdnu** |Dny v týdnu, úloha se bude spouštět. Lze zadat pouze s frekvencí týdně. |<ul><li>"Pondělí", "Úterý", "Středa", "Čtvrtek", "Pátek", "Sobota" a "Neděle"</li><li>Pole žádnou z výše uvedené hodnoty (pole maximální velikost 7)</li></ul>*Není* malá a velká písmena |
| **monthlyOccurrences** |Určuje, které dny v měsíci má být úloha spuštěna. Lze zadat pouze s měsíční četností. |<ul><li>Pole objektů monthlyOccurrence:</li></ul> <pre>{ "day": *day*,<br />  "occurrence": *occurrence*<br />}</pre><p> *den* je den v týdnu, úloha se spustí, například {neděle} je každou neděli v měsíci. Vyžaduje se.</p><p>Výskyt je *výskyt* dne v měsíci, například {neděle, -1} je poslední neděle v měsíci. Volitelné.</p> |
| **Prescribed** |Den v měsíci, který má být úloha spuštěna. Lze zadat pouze s měsíční četností. |<ul><li>Libovolná hodnota < = -1 a > =-31.</li><li>Libovolná hodnota > = 1 a < = 31.</li><li>Pole výše hodnoty</li></ul> |
...
| <code>{"minutes":[15]}</code> |Spustit na 15 minut po hodině každou hodinu. Spouští každou hodinu od 00:15 AM, 1:15:00, 2:15:00, atd. a končí na 22:15:00 a 23:15:00. |
...
| <code>{"minutes":[0,15,30,45], "weekDays":["monday", "tuesday", "wednesday", "thursday", "friday"]}</code> |Spustit každých 15 minut ve všední dny |
