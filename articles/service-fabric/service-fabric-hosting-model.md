...
>
> - Všechny používá aplikace word *repliky* odkazuje na obou repliku stavové služby nebo instance statless služby.
...
Jak vidíte, Service Fabric aktivovat dva nové kopie 'MyServicePackage' (jeden pro každou repliku z oddílu **P6** & **N7**) a umístit každou repliku v jeho vyhrazené kopie *CodePackage*. Další fakt, který je zde, při **výhradní proces** model se používá, pro daný *aplikace*, více zkopíruje z danou *ServicePackage* může být aktivní v  *Uzel*. Ve výše příkladu vidíme pro aktivní tři kopie 'MyServicePackage' **fabric: / App1**. Každá z těchto aktivní kopie 'MyServicePackage' má **ServicePackageAtivationId** spojené s ním, které identifikuje tuto kopii v rámci *aplikace* **fabric: / App1**.
...
> [!NOTE]
>- **Sdílený proces** model hostování odpovídá **ServicePackageAtivationMode** rovna **SharedProcess**. Toto je výchozí model hostování a **ServicePackageAtivationMode** není nutné zadávat při vytváření služby.
>
>- **Výhradní proces** model hostování odpovídá **ServicePackageAtivationMode** rovna **ExclusiveProcess** a je třeba explicitně zadat při vytváření služby. 
>
>- Model hostování služby může být známé pomocí dotazu [popisu služby] [ p2] a prohlížení hodnotu **ServicePackageAtivationMode**.
>
>
...
>
> - Pokud **ServicePackageActivationId** je ommited výchozí hodnota "prázdný řetězec". Pokud službu nasazenou balíčku, který byl aktivován pod **sdílený proces** modelu je k dispozici, pak se provede operaci na něm, jinak se operace nezdaří.
>
...
Ale povolit speciální scénáře, kde jeden *ServiceType* za *ServicePackage* nemusí být žádoucí, funkčně, Service Fabric umožňuje mít víc než jeden *ServiceType* za *ServicePackage* (a jeden *CodePackage* můžete zaregistrovat více než jeden *ServiceType*). Následují některé scénáře, kde může být užitečné tyto konfigurace:
