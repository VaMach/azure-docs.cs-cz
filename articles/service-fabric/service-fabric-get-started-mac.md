...
Pomocí Mac OS X můžete sestavit aplikace Service Fabric, které poběží na clusterech s Linuxem. Tento dokument popisuje nastavení počítače Mac pro vývoj.

## <a name="prerequisites"></a>Požadavky
Service Fabric nefunguje v OS X nativně. Pro spuštění místního clusteru Service Fabric se poskytuje předkonfigurovaná image kontejneru Dockeru. Než začnete, budete potřebovat:
...
2. Následujícím způsobem aktualizujte konfiguraci démona Dockeru na hostiteli a potom démon Dockeru restartujte: 
    ...
    ```
    Tuto aktualizaci můžete provést přímo v souboru daemon.json na instalační cestě Dockeru (její umístění se může u jednotlivých počítačů lišit, například ~/Library/Containers/com.docker.docker/Data/database/com.docker.driver.amd64-linux/etc/docker/daemon.json). Doporučený postup aktualizace vypadá takto: Přejděte na ikonu Dockeru > Předvolby > Démon > Upřesnit a aktualizaci proveďte tady.
...
6. Po úspěšném dokončení kroku 5 můžete na svém počítači Mac přejít na adresu ``http://localhost:19080``, kde by se měl zobrazit Service Fabric Explorer.
...
1. Na počítači Mac musí být nainstalováno Node.js a NPM. Pokud ne, můžete nainstalovat Node.js a NPM pomocí Homebrew následujícím způsobem:
