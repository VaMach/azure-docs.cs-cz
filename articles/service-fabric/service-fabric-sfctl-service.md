...
| Název – dns            | Název DNS služby, který se má vytvořit. Služba system Service Fabric DNS musí být povolena pro toto nastavení.|
| – počet instancí      | Počet instancí. To se týká pouze bezstavové služby.|
| int – schéma          | Označuje, že služba by měla být jednotně rozděleného mezi rozsah celých čísel bez znaménka.|
...
| int –. schéma nízká      | Začátek rozsahu klíče celých čísel, pokud se používá schéma oddílu uniform celé číslo.|
...
| --s názvem schématu        | Označuje, že služba by měla mít více oddílů s názvem.|
| --s názvem seznam schéma   | JSON kódovaný seznamu názvů oddílu služba napříč, pokud se používá schéma s názvem oddílu.|
...
| --singleton – schéma    | Určuje, služba by měl mít jeden oddíl nebo jinou hodnotu než oddílů služby.|
