...
|(bez názvu)|Symbol v tomto sloupci obvykle označuje, že pro vybranou tabulku nelze povolit funkci Stretch kvůli potížím s blokováním. Důvodem může být to, že tabulka využívá nepodporovaný datový typ. Pokud na symbol najedete myší, zobrazí se další informace ve formě popisu tlačítka. Další informace najdete v tématu [Surface area limitations and blocking issues for Stretch Database](sql-server-stretch-database-limitations.md) (Omezení možností útoku a blokující problémy pro funkci Stretch Database).|
|**Roztažení**|Udává, jestli je příslušná tabulka už povolená.|
|**Migrace**|Můžete migrovat celou tabulku (**Celá tabulka**) nebo v průvodci zadat predikát filtru na základě data. Pokud chcete pro výběr řádků k migraci využít jiný predikát filtru, po ukončení tohoto průvodce spusťte příkaz ALTER TABLE a zadejte požadovaný predikát filtru. Další informace o predikátu filtru najdete v tématu [Použití predikátu filtru k výběru řádků pro migraci (Stretch Database)](sql-server-stretch-database-predicate-function.md). Další informace o použití predikátu najdete v tématu [Povolení funkce Stretch Database pro tabulku](sql-server-stretch-database-enable-table.md) nebo [ALTER TABLE (Transact-SQL)](https://msdn.microsoft.com/library/ms190273.aspx).|
|**Řádky**|Určuje počet řádků v tabulce.|
|**Velikost (kB)**|Určuje velikost tabulky v kB.|

## <a name="Filter"></a>Volitelně zadejte predikát filtru na základě data.

Pokud chcete pomocí predikátu filtru na základě data vybrat řádky k migraci, proveďte na stránce **Vybrat tabulky** následující akce.
...
1.  V seznamu **Select the tables you want to stretch** (Vyberte tabulky, které chcete roztáhnout) klikněte v řádku pro příslušnou tabulku na **Entire Table** (Celá tabulka). Otevře se dialogové okno **Select rows to stretch** (Vybrat řádky k roztažení).
    ...
    ![Definice predikátu filtru na základě data][StretchWizardImage2a]

2.  V dialogovém okně **Select rows to stretch** (Vybrat řádky k roztažení) vyberte **Choose Rows** (Zvolit řádky).

3.  V **poli pro název** zadejte název predikátu filtru.

4.  Pro klauzuli **Where** vyberte v tabulce sloupec data, vyberte operátor a potom zadejte hodnotu data.

5. Kliknutím na **Check** (Zkontrolovat) predikát otestujte. Pokud predikát vrátí výsledky z tabulky (to znamená, pokud v tabulce jsou řádky k migraci, které splňují zadanou podmínku), test vrátí **Úspěch**.

6.  Kliknutím na Done (Hotovo) se vrátíte na stránku **Select tables** (Vybrat tabulky).

    ![Stránka pro výběr tabulek po definování predikátu filtru][StretchWizardImage2b]
...
2.  Vyberte předplatné Azure, které chcete použít pro funkci Stretch Database.

3.  Vyberte oblast Azure. Pokud vytvoříte nový server, vytvoří se v této oblasti.
...
4.  Určete, jestli chcete použít existující server, nebo vytvořit nový server Azure.
    ...
    -   **Existující server**
        ...
        1.  Vyberte nebo zadejte název existujícího serveru Azure.

        2.  Vyberte metodu ověřování.

            -   Pokud vyberete **Ověřování serveru SQL Server**, vytvořte nové přihlašovací jméno a heslo.

            -   Pokud chcete ke komunikaci se vzdáleným serverem Azure využít účet federované služby pro SQL Server, vyberte **Integrované ověřování Active Directory**.
...
Zadejte silné heslo pro vytvoření hlavního klíče databáze. Pokud hlavní klíč databáze již existuje, zadejte heslo pro tento klíč.
...
Použijte veřejnou IP adresu SQL Serveru (nebo zadejte rozsah IP adres) a vytvořte v Azure pravidlo brány, které SQL Serveru umožňuje komunikovat se vzdáleným serverem Azure.
...
Zkontrolujte hodnoty, které jste zadali, a možnosti, které jste vybrali v průvodci. Potom povolte funkci Stretch volbou **Dokončit**.
...
Volitelně vyberte **Monitor**, pokud chcete spustit sledování stavu migrace dat pomocí funkce Stretch Database Monitor. Další informace najdete v tématu věnovaném [monitorování a řešení potíží s migrací dat (Stretch Database)](sql-server-stretch-database-monitor.md).
...
-   [Monitorování funkce Stretch Database](sql-server-stretch-database-monitor.md) a zobrazení stavu migrace dat
...
-   [Zálohování a obnovení databází s povolenou funkcí Stretch](sql-server-stretch-database-backup.md)
