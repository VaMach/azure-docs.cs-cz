...
3. Open WorkerRoleB.cs and examine the code.
	...
	When a queue item is pulled from the queue by the [GetMessage][]  method, that queue item becomes invisible for 30 seconds to all other worker and web roles accessing the queue. This is what ensures that only one worker role instance will pick up any given queue message for processing. You can explicitly set this *exclusive lease* time (the time the queue item is invisible) by passing a  [visibility timeout](http://msdn.microsoft.com/zh-cn/library/windowsazure/ee758454.aspx) parameter to the  `GetMessage` method. If the worker role could take more than 30 seconds to process a queue message, you should increase the exclusive lease time to prevent other role instances from processing the same message. 
...
* [How to use the Table Storage Service in .NET](http://www.windowsazure.com/zh-cn/develop/net/how-to-guides/table-services/) 
...
* [Real World: Designing a Scalable Partitioning Strategy for Azure Table Storage](http://msdn.microsoft.com/zh-cn/library/windowsazure/hh508997.aspx)
...
[tut3configstorage]: /zh-cn/develop/net/tutorials/multi-tier-web-site/3-web-role/#configstorage
[tut4]: /zh-cn/develop/net/tutorials/multi-tier-web-site/4-worker-role-a/
[queuehowto]: /zh-cn/develop/net/how-to-guides/queue-service/

[blobhowto]: /zh-cn/develop/net/how-to-guides/blob-storage/
[GetMessage]: http://msdn.microsoft.com/zh-cn/library/windowsazure/ee741827.aspx
[getstartedtutorial]: /zh-cn/develop/net/tutorials/get-started
...
[autoscaling-and-windows-azure]: http://msdn.microsoft.com/zh-cn/library/hh680945(v=PandP.50).aspx
