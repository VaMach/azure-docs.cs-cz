...
For example, to point the main company domain **www.contoso.com** to a Traffic Manager domain named **contoso.trafficmanager.cn**, update the DNS resource record as shown below:
``www.contoso.com IN CNAME contoso.trafficmanager.cn``

All traffic going to *www.contoso.com* will now redirect to *contoso.trafficmanager.cn*. Be sure that you are using a domain where you want all traffic redirected to Traffic Manager. 
...
>3. Type the command ``nslookup <your traffic manager domain>``.
 For example, the following command checks the domain with the prefix *myapp.contoso* ``nslookup myapp.contoso.trafficmanager.cn``
...
For example, the following command routes all traffic going to **www.contoso.com** to the Traffic Manager domain **contoso.trafficmanager.cn**: 
`` www.contoso.com IN CNAME contoso.trafficmanager.cn``
