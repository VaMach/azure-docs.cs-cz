...
## <a name="addref"></a><span class="short-header">Add project reference</span>Add a reference to the web project
...
## <a name="addmodel"></a>Add the SendEmail model
...
## <a name="addcode"></a><span class="short-header">Add worker role code</span>Add code that runs when the worker role starts
...
There is a minimal charge for every Azure Storage query, even if it doesn't return any data, so continuously re-scanning would unnecessarily add to your Azure expenses. As this tutorial is being written, the cost is $0.10 per million transactions (a query counts as a transaction), so the sleep time could be made much less than a minute and the cost of scanning the tables for messages to be sent would still be minimal. For more information about pricing, see the [first tutorial][first tutorial in the series].
...
## <a name="testing"></a>Test worker role A
...
For links to additional resources for working with Azure Storage tables, queues, and blobs, see [the last tutorial in this series][next tutorial].
...
<div><a href="/en-us/develop/net/tutorials/multi-tier-web-site/5-worker-role-b/" class="site-arrowboxcta download-cta">Tutorial 5</a></div>
  ...
  [Message row]: ./media/cloud-services-dotnet-multi-tier-app-storage-1-worker-role-a/mtas-worker-a-test-ase-message-table.png
  ...
  [next tutorial]: /en-us/develop/net/tutorials/multi-tier-web-site/5-worker-role-b/
