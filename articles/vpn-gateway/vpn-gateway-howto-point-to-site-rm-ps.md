---
title: "Připojení počítače k virtuální síti Azure typu Point-to-Site s použitím ověření certifikátu: PowerShell | Dokumentace Microsoftu"
description: "Připojte počítač bezpečně k virtuální síti vytvořením připojení brány VPN typu Point-to-Site s použitím ověření certifikátu. Tento článek se týká modelu nasazení Resource Manager a používá PowerShell."
...
Tento článek ukazuje postup vytvoření virtuální sítě s připojením typu Point-to-Site v modelu nasazení Resource Manager pomocí PowerShellu. Tato konfigurace používá certifikáty k ověření připojujícího se klienta. Tuto konfiguraci můžete vytvořit také pomocí jiného nástroje nasazení nebo pro jiný model nasazení, a to výběrem jiné možnosti z následujícího seznamu:
...
3. Vytvořte virtuální síť.
  ...
  V tomto příkladu je server DNS volitelný. Zadání hodnoty nevytvoří nový server DNS. Server DNS, jehož IP adresu zadáte, by měl být server DNS, který dokáže přeložit názvy pro prostředky, ke kterým se připojujete. V tomto příkladu jsme použili privátní IP adresu, ale je pravděpodobné, že to není IP adresa vašeho serveru DNS. Je potřeba, abyste použili svoje vlastní hodnoty.
...
## <a name="clientconfig"></a>7. Stažení konfiguračního balíčku klienta VPN
