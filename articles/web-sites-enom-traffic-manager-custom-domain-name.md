...
4. The Host Records editor allows you to select the specific record type using the **Record Type** field. For Azure Web Sites that use Traffic Manager, you should only use the **CNAME (Alias)** selection, as Traffic Manager only works with CNAME records.
	...
	* When adding a CNAME record, you must set the **Host Name** field to the sub-domain you wish to use. For example, **www**. You must set the **Address** field to the **.trafficmanager.cn** domain name of the Traffic Manager profile used with your Azure Web Site. For example, **contoso.trafficmanager.cn**.
