# Přehled
## [Přehled aplikace logiky](logic-apps-what-are-logic-apps.md)
## [Konektory](../connectors/apis-list.md)
## [Enterprise Integration Pack](logic-apps-enterprise-integration-overview.md)

# Začínáme
## [Vytvoření aplikace logiky](logic-apps-create-a-logic-app.md)
## [Použití funkcí logické aplikace](logic-apps-use-logic-app-features.md)
## [Přehled šablon](logic-apps-use-logic-app-templates.md)
## [Vytvoření aplikace pomocí šablony](logic-apps-arm-provision.md)
## [Příklady a scénáře](logic-apps-examples-and-scenarios.md)
## [Scénář služby Service Bus](logic-apps-scenario-function-sb-trigger.md)
## [Zpracování B2B](logic-apps-enterprise-integration-b2b.md)
## [Zpracování XML](logic-apps-enterprise-integration-xml.md)

# Postup
## Sestavení
### [Vytvoření konektoru](logic-apps-create-api-app.md)
### [Použití konektoru systému souborů](logic-apps-using-file-connector.md)
### [Použití konektoru SAP](logic-apps-using-sap-connector.md)
### [Nástroje sady Visual Studio](logic-apps-deploy-from-vs.md)
### [Použití Azure Functions](logic-apps-azure-functions.md)
### [Aplikace logiky jako volatelné koncové body](logic-apps-http-endpoint.md)
### [Smyčky, obory a rozdělení dávek](logic-apps-loops-and-scopes.md)
### [Použití vlastních rozhraní API](logic-apps-custom-hosted-api.md)
### [Použití příkazů switch](logic-apps-switch-case.md)
### [Zpracování výjimek](logic-apps-exception-handling.md)
### [Scénář zpracování výjimek](logic-apps-scenario-error-and-exception-handling.md)
### [Zpracování typů obsahu](logic-apps-content-type.md)
### [Definice pracovního postupu](logic-apps-author-definitions.md)
### [Rady pro řešení potíží](logic-apps-diagnosing-failures.md)
### [Omezení a konfigurace](logic-apps-limits-and-config.md)


## Použití sady Enterprise Integration Pack (EIP)
### [Přehled účtů pro integraci](logic-apps-enterprise-integration-create-integration-account.md)
### [Smlouvy](logic-apps-enterprise-integration-agreements.md)
### [Zpracování B2B](logic-apps-enterprise-integration-b2b.md)
### [Zpracování XML](logic-apps-enterprise-integration-xml.md)
### [Zpracování plochých souborů](logic-apps-enterprise-integration-flatfile.md)
### [Přidaní map XSLT](logic-apps-enterprise-integration-maps.md)
### [Transformace XML](logic-apps-enterprise-integration-transform.md)
### [Ověření XML](logic-apps-enterprise-integration-xml-validation.md)
### [Použití certifikátů](logic-apps-enterprise-integration-certificates.md)
### [Přidání partnerů](logic-apps-enterprise-integration-partners.md)
### [Přidání schémat](logic-apps-enterprise-integration-schemas.md)
### [Metadata úložiště](logic-apps-enterprise-integration-metadata.md)
### [Integrace AS2](logic-apps-enterprise-integration-as2.md)
### [Kódování AS2](logic-apps-enterprise-integration-as2-encode.md)
### [Dekódování AS2](logic-apps-enterprise-integration-as2-decode.md)
### [Integrace EDIFACT](logic-apps-enterprise-integration-edifact.md)
### [Kódování EDIFACT](logic-apps-enterprise-integration-edifact-encode.md)
### [Dekódování EDIFACT](logic-apps-enterprise-integration-edifact-decode.md)
### [Integrace X12](logic-apps-enterprise-integration-x12.md)
### [Kódování X12](logic-apps-enterprise-integration-x12-encode.md)
### [Dekódování X12](logic-apps-enterprise-integration-x12-decode.md)


## Použití brány
### [Připojení k místním prostředkům](logic-apps-gateway-connection.md)
### [Instalace](logic-apps-gateway-install.md)

## Automatizace
### [Šablona Automation](logic-apps-create-deploy-template.md)
### [Publikování z VS](logic-apps-deploy-from-vs.md)

## Spravovat
### [Použití průzkumníka Visual Studio Cloud Explorer](logic-apps-manage-from-vs.md)
### [Zabezpečení aplikací logiky](logic-apps-securing-a-logic-app.md)
### [Monitorování Logic Apps](logic-apps-monitor-your-logic-apps.md)
### [Monitorování zpráv B2B](logic-apps-monitor-b2b-message.md)
#### [Schéma sledování AS2](logic-apps-track-integration-account-as2-tracking-schemas.md)
#### [Schéma sledování X12](logic-apps-track-integration-account-x12-tracking-schema.md)
#### [Vlastní schémata sledování](logic-apps-track-integration-account-custom-tracking-schema.md)
#### [Sledování zpráv B2B](logic-apps-track-b2b-messages-omsportal.md)
#### [Dotazy na portál OMS](logic-apps-track-b2b-messages-omsportal-query-filter-control-number.md)

### [Účty pro integraci](logic-apps-enterprise-integration-accounts.md)
### [Ceny](logic-apps-pricing.md)

# Referenční informace
## [PowerShell](/powershell/resourcemanager/azurerm.logicapp/v2.3.0/azurerm.logicapp)
## Historie schématu
### [GA](logic-apps-schema-2016-04-01.md)
### [Preview](logic-apps-schema-2015-08-01.md)
## [REST](/rest/api/logic/)

# Zdroje a prostředky
## [Ceny](https://azure.microsoft.com/pricing/details/logic-apps/)  
## [Fórum MSDN](https://social.msdn.microsoft.com/Forums/azure/en-US/home?forum=azurelogicapps)
## [Stack Overflow](http://stackoverflow.com/questions/tagged/azure-logic-apps)
## [Aktualizace služeb](https://azure.microsoft.com/updates/?product=logic-apps)


<!--HONumber=Feb17_HO3-->


