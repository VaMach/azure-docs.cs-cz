...
3. Klikněte na aktivitu **Write-Output** na plátně. Tato akce otevře okno pro kontrolu konfigurace, které umožňuje aktivitu konfigurovat.
4. **Popisek** obsahuje ve výchozím nastavení název rutiny, ale můžete ho změnit na něco popisnějšího. Změňte ho na *Textový výstup Hello World*.
...
6. Vyberte parametr **InputObject**. V tomto parametru budete zadávat text, který se odešle do výstupního datového proudu.
7. V rozevíracím seznamu **Zdroj dat** vyberte **Powershellový výraz**. Rozevírací seznam **Zdroj dat** obsahuje různé zdroje, které můžete použít k naplnění hodnoty parametru.
   ...
   Můžete použít výstup z těchto zdrojů, například další aktivitu, asset Automation nebo powershellový výraz. V tomto případě je výstupem jenom *Hello World*. Můžete použít powershellový výraz a zadat řetězec.
...
4. Po dokončení úlohy runbooku se zobrazí jeho výstup. V tomto případě se zobrazí *Hello World*.<br> ![Hello World](media/automation-first-runbook-graphical/runbook-test-results.png)
...
4. Spusťte runbook kliknutím na **Spustit** a po zobrazení výzvy na **Ano**.<br> ![Spuštění runbooku](media/automation-first-runbook-graphical/runbook-controls-start-revised20165.png)
5. Otevře se okno úlohy vytvořeného runbooku. Ověřte, že se jako **Stav úlohy** zobrazuje **Dokončeno**.
7. Když se jako stav runbooku zobrazí *Dokončeno*, klikněte na **Výstup**. Otevře se okno **Výstup** a v podokně se zobrazí text *Hello World*.
8. Zavřete okno Výstup.
9. Klikněte na **Všechny protokoly** a otevřete okno Datové proudy, které patří k úloze runbooku. Ve výstupním datovém proudu byste měli vidět jenom text *Hello World*, ale můžou se zobrazit i jiné datové proudy z úlohy runbooku, například Podrobný nebo Chyba, pokud do nich runbook zapisuje.
10. Zavřete okno Všechny protokoly a okno Úloha a vraťte se do okna MyFirstRunbook.
11. Pokud chcete zobrazit všechny úlohy runbooku, zavřete okno **Úloha** a v části **PROSTŘEDKY** vyberte **Úlohy**. Vypíšou se všechny úlohy, které tento runbook vytvořil. Ve výpisu by se měla zobrazit pouze jedna úloha, protože jste ji spustili jenom jednou.
...
1. V okně Účty Automation v části **SDÍLENÉ PROSTŘEDKY** vyberte **Proměnné**.
2. Klikněte na **Přidat proměnnou**.<br>![Proměnná služby Automation](media/automation-first-runbook-graphical/create-new-subscriptionid-variable.png)
...
1. Přejděte zpět ke svému runbooku a v okně MyFirstRunbook vyberte **Upravit**.<br> ![Úprava runbooku](media/automation-first-runbook-graphical/runbook-controls-edit-revised20165.png)
2. **Textový výstup Hello World** už nepotřebujete, takže klikněte na tři tečky a vyberte **Odstranit**.
3. V ovládacím prvku Knihovna rozbalte **ASSETY**, **Připojení** a výběrem možnosti **Přidat na plátno** přidejte **AzureRunAsConnection** na plátno.
...
10. **Add-AzureRmAccount** obsahuje několik sad parametrů, takže před zadáním hodnot parametrů musíte nejprve jednu sadu vybrat. Klikněte na **Sada parametrů** a potom vyberte sadu **ServicePrincipalCertificate**.
...
12. V okně Hodnota parametru v části **Zdroj dat** vyberte **Výstup aktivity**, v seznamu vyberte **Get Run As Connection**, do textového pole **Cesta pole** zadejte **ApplicationId** a potom klikněte na tlačítko **OK**. Název vlastnosti pro cestu pole zadáváte kvůli tomu, že výstupem aktivity je objekt s více vlastnostmi.
...
20. **Set-AzureRmContext** obsahuje několik sad parametrů, takže před zadáním hodnot parametrů musíte nejprve jednu sadu vybrat. Klikněte na **Sada parametrů** a potom vyberte sadu **SubscriptionId**.  
...
5. Vyberte **Název**. V části **Zdroj dat** vyberte **Powershellový výraz** a zadejte název virtuálního počítače (v uvozovkách), který pomocí tohoto runbooku spustíte. Klikněte na **OK**.<br>![Hodnota parametru názvu Start-AzureRmVM](media/automation-first-runbook-graphical/runbook-startvm-nameparameter.png)
...
7. Klikněte na testovací podokno, abyste mohli runbook otestovat.
