---
title: "Vytvoření klíčů SSH na Linuxu a Macu | Dokumentace Microsoftu"
...
---
# <a name="create-ssh-keys-on-linux-and-mac-for-linux-vms-in-azure"></a>Vytvoření klíčů SSH na Linuxu a Macu pro virtuální počítače s Linuxem v Microsoft Azure
...
V následujících příkazech nahraďte příklady vlastními volbami.
...
## <a name="create-the-ssh-keys"></a>Vytvoření klíčů SSH
...
## <a name="using-sshkeygen"></a>Použití příkazu ssh-keygen
...
## <a name="example-of-sshkeygen"></a>Příklad ssh-keygen
```bash
ssh-keygen -t rsa -b 2048 -C "myusername@myserver"
...
## <a name="using-sshagent-to-store-your-private-key-password"></a>Použití příkazu ssh-agent k uložení hesla k soukromému klíči 
...
`Host`= název hostitele volaného na terminálu.  `Host fedora22`říká protokolu `ssh fedora22`, aby použil hodnoty v bloku nastavení s popiskem `SSH`. POZNÁMKA: Může to být libovolný popisek, který pro vaše použití dává smysl a který nepředstavuje skutečný název hostitele žádného serveru.

`Hostname 102.160.203.241`= IP adresa nebo název DNS serveru, ke kterému přistupujete.

`User myusername` = vzdálený uživatelský účet, který se má používat při přihlašování k serveru.

`PubKeyAuthentication yes`= říká protokolu SSH, že chcete k přihlášení použít klíč SSH.

`IdentityFile /home/myusername/.ssh/id_id_rsa`= privátní klíč SSH a odpovídající veřejný klíč pro ověřování.
...
* [Vytvoření zabezpečeného virtuálního počítače s Linuxem pomocí rozhraní příkazového řádku Azure](virtual-machines-linux-quick-create-cli.md)
