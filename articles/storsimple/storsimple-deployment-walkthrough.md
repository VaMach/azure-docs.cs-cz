...
| --- | --- | --- |
| **IP**</br>**Podsíť**</br>**Brána** |Je toto platná adresa IPv4 nebo IPv6?</br>Je toto platná podsíť?</br>Je toto platná brána?</br>Je tato IP adresa v síti duplicitní? |`ping ip`</br>`arp -a`</br>Příkazy `ping` a `arp` by měly vrátit chybu, což ukazuje, že v síti datového centra není žádné zařízení používající tuto IP adresu. |
|  | | |
