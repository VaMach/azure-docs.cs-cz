...
This article provides instructions on using a custom domain name purchased from [Moniker.com](https://moniker.com) with Azure Web Sites.
...
5. Then create a DNS record by first selecting the **Record Type**. Then fill in the **Hostname** and the **Address**. 
    ...
    * When adding a CNAME record, you must set the **Hostname** field to the sub-domain you wish to use. For example, **www**. You must set the **Address** field to the **.chinacloudsites.cn** domain name of your Azure Web Site. For example, **contoso.chinacloudsites.cn**.
    ...
    * When adding an A record, you must set the **Hostname** field to either **@** (this represents root domain name, such as **contoso.com**,) or the sub-domain you wish to use (for example, **www**.) You must set the **Address** field to the IP address of your Azure Web Site.
		...
		> 
		> * A **Hostname** value of **www** with an **Address** value of **&lt;yourwebsitename&gt;.chinacloudsites.cn**.
		> 
		> OR
		> 
		> * A **Hostname** value of **awverify.www** with an **Address** value of **awverify.&lt;yourwebsitename&gt;.chinacloudsites.cn**.
		> 
