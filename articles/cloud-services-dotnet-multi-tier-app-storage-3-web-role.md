...
Trace data is not saved when called from the `OnStop` method without performing an [On-Demand Transfer](http://msdn.microsoft.com/zh-cn/library/windowsazure/gg433075.aspx). You can view the `OnStop` trace information in real time with the  [dbgview](http://technet.microsoft.com/zh-cn/sysinternals/bb896647.aspx) utility from a remote desktop connection.
...
3. Open *MailingListController.cs* and examine the code.
	...
	The only difference in the two queries is the model type that they pass to the [TableOperation.Retrieve](http://msdn.microsoft.com/zh-cn/library/windowsazure/microsoft.windowsazure.storage.table.tableoperation.retrieve.aspx) method. The model type specifies the schema (the properties) of the row or rows that you expect the query to return. A single table may have different schemas in different rows. Typically you specify the same model type when reading a row that was used to create the row.
...
<div><a href="/zh-cn/develop/net/tutorials/multi-tier-web-site/4-worker-role-a/" class="site-arrowboxcta download-cta">Tutorial 4</a></div>
...
[tut5ns]: /zh-cn/develop/net/tutorials/multi-tier-web-site/5-worker-role-b/#nextsteps
...
[nexttutorial]: /zh-cn/develop/net/tutorials/multi-tier-web-site/4-worker-role-a/
...
[tabledatamodel]: http://msdn.microsoft.com/zh-cn/library/windowsazure/dd179338.aspx 
