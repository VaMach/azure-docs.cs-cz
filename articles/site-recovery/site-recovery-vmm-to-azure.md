...
---
# <a name="replicate-hyperv-virtual-machines-in-vmm-clouds-to-azure-using-the-azure-portal"></a>Replikace virtuálních počítačů Hyper-V v cloudech VMM do Azure pomocí webu Azure Portal
...
| **Oblast** | **Podrobnosti** |
| --- | --- |
| **Scénář nasazení** |Replikace virtuálních počítačů Hyper-V v cloudech VMM do Azure pomocí webu Azure Portal |
| **Místní požadavky** |Jeden nebo více serverů VMM běžících na řešení System Center 2012 R2 s jedním nebo více cloudy.<br/><br/> V cloudech musí být jedna nebo více skupin hostitelů VMM.<br/><br/> Aspoň jeden server Hyper-V, na kterém je minimálně Windows Server 2012 R2 s rolí Hyper-V nebo Microsoft Hyper-V Server 2012 R2 s nejnovějšími aktualizacemi.<br/><br/> Servery VMM a hostitelé Hyper-V musí mít přístup k internetu a musí mít možnost přistupovat ke konkrétním adresám URL, a to buď přímo, nebo přes proxy server. [Úplné podrobnosti](#on-premises-prerequisites) |
...
Tady je seznam toho, co je potřeba v Azure.
...
## <a name="onpremises-prerequisites"></a>Místní požadavky
...
#### <a name="commandline-installation-for-the-azure-site-recovery-provider"></a>Instalace zprostředkovatele Azure Site Recovery pomocí příkazového řádku
...
### <a name="install-the-azure-recovery-services-agent-on-hyperv-hosts"></a>Instalace agenta Služeb zotavení Azure na hostitele Hyper-V
...
#### <a name="set-up-internet-proxy-access-to-site-recovery-from-hyperv-hosts"></a>Nastavení internetového přístupu proxy serveru k Site Recovery z hostitelů Hyper-V
