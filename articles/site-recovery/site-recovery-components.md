...
- **Replikace fyzických počítačů**: Fyzické počítače s Windows nebo Linuxem můžete replikovat buď do Azure, nebo do sekundárního datacentra. Proces pro replikování fyzických počítačů je téměř stejný jako proces pro replikaci virtuálních počítačů VMware.
...
- **Replikace virtuálních počítačů Hyper-V spravovaných v cloudech System Center VMM**: Místní virtuální počítače Hyper-V v cloudech VMM, které běží na hostitelských serverech Hyper-V, můžete replikovat buď do Azure, nebo do sekundárního datacentra. Můžete replikovat pomocí standardní repliky Hyper-V nebo replikace sítě SAN.
...
* **Pomocí webu Azure Portal** – Pokud nasadíte službu Site Recovery na webu Azure Portal, můžete převzít služby virtuálních počítačů do klasického úložiště správce služeb nebo do Resource Manageru. Replikace virtuálních počítačů VMware v portálu Azure přináší řadu výhod, včetně možnosti replikace do klasického úložiště nebo do úložiště Správce prostředků v Azure. [Další informace](site-recovery-vmware-to-azure.md).
* **Použití portálu Classic**-Službu Site Recovery můžete nasadit na portálu Classic. V tomto nasazení můžete předat virtuální počítače jenom do klasického úložiště v Azure a nikoli do úložiště Správce prostředků. [Další informace](site-recovery-vmware-to-azure-classic.md).
...
* Pokud provádíte nasazení na webu Azure Portal, můžete replikovat do úložiště založeného na Správci prostředků a použít sítě Správce prostředků pro připojení virtuálních počítačů Azure po převzetí služeb při selhání.
* Když provádíte nasazení na webu Azure Portal, je podporováno úložiště LRS i GRS. Na portálu Classic je vyžadováno úložiště GRS.
...
- **Úložiště Azure:** K ukládání replikovaných dat potřebujete účet úložiště Azure. Můžete použít klasický účet nebo účet úložiště Správce prostředků. Při nasazení na portálu Azure může být účet LRS nebo GRS. Replikovaná data jsou uložena v úložišti Azure a virtuální počítače Azure jsou vytvářeny, když dojde k převzetí služeb při selhání.
- **Síť Azure:** Potřebujete virtuální síť Azure, ke které se budou virtuální počítače Azure připojovat, když dojde k jejich vytvoření při převzetí služeb při selhání. Na webu Azure Portal je možné vytvářet sítě v klasickém modelu správy služeb nebo v modelu Resource Manageru.
- **Místní konfigurační server**: Potřebujete místní počítač Windows Server 2012 R2, na kterém běží konfigurační server a další součásti Site Recovery. Pokud provádíte replikaci virtuálních počítačů VMware, mělo by se jednat o vysoce dostupný virtuální počítač VMware. Pokud chcete replikovat fyzické servery, může se jednat o fyzický počítač. Na počítač se nainstalují tyto součásti služby Site Recovery:
...
- **Virtuální počítače VMware**: Na každý počítač, který chcete replikovat do Azure, bude nutné nainstalovat součást služby Mobility. Tato služba zaznamenává datové zápisy na počítači a předává je na procesní server. Tuto součást můžete nainstalovat ručně nebo ji může automaticky (v režimu nabízení) nainstalovat procesní server, když pro počítač aktivujete replikaci.
...
* [Další informace](site-recovery-vmware-to-azure-classic.md#before-you-start-deployment) o požadavcích na nasazení do portálu Azure Classic.
...
Následujícím způsobem můžete do Azure pomocí Site Recovery replikovat virtuální počítače Hyper-V, které nejsou spravovány v cloudu System Center VMM:

- **Pomocí webu Azure Portal** – Pokud provádíte nasazení Site Recovery na webu Azure Portal, můžete převzít služby virtuálních počítačů do klasického úložiště nebo do Správce prostředků. [Další informace](site-recovery-hyper-v-site-to-azure.md).
- **Pomocí portálu Classic** - Službu Site Recovery můžete nasadit na portálu Classic. V tomto nasazení můžete předat virtuální počítače jenom do klasického úložiště v Azure, a nikoli do úložiště Správce prostředků. [Další informace](site-recovery-hyper-v-site-to-azure-classic.md).

Architektura je pro obě nasazení podobná s následující výjimkou:

* Pokud provádíte nasazení na webu Azure Portal, můžete replikovat do úložiště Správce prostředků a použít sítě Správce prostředků pro připojení virtuálních počítačů Azure po převzetí služeb při selhání.
* Proces nasazení je na portálu Azure jednodušší a srozumitelnější.

Zde je seznam toho, co budete potřebovat:

* **Účet Azure**: Potřebujete účet Microsoft Azure.
* **Úložiště Azure:** K ukládání replikovaných dat potřebujete účet úložiště Azure. Na webu Azure Portal můžete použít klasický účet nebo účet úložiště Správce prostředků. Na portálu Classic můžete použít pouze klasický účet. Replikovaná data jsou uložena v úložišti Azure a virtuální počítače Azure jsou vytvářeny, když dojde k převzetí služeb při selhání.
* **Síť Azure**: Potřebujete síť Azure, ke které se budou spuštěné virtuální počítače Azure připojovat, když dojde k jejich vytvoření po převzetí služeb při selhání.
* **Hostitel Hyper-V**: Potřebujete minimálně jeden hostitelský server Windows Server 2012 R2 Hyper-V. Během nasazování Site Recovery nainstalujete na hostitele zprostředkovatele služby Azure Site Recovery a agenta služby Microsoft Azure Recovery Services.
* **Virtuální počítače Hyper-V**: Potřebujete jeden nebo více virtuálních počítačů na hostitelském serveru Hyper-V. Zprostředkovatel Azure Site Recovery a agent služby Azure Recovery Services se nainstalují do hostitele Hyper-V během nasazení Site Recovery. Zprostředkovatel koordinuje a orchestruje replikaci pomocí služby Site Recovery přes internet. Agent zpracovává replikovaná data dat přes HTTPS 443. Komunikace z poskytovatele i agenta je zabezpečená a šifrovaná. Šifrují se rovněž replikovaná data v úložišti Azure.
...
* **Pomocí webu Azure Portal** – Pokud provádíte nasazení Site Recovery na webu Azure Portal, můžete převzít služby virtuálních počítačů do klasického úložiště nebo do Správce prostředků. [Další informace](site-recovery-vmm-to-azure.md).
...
* **Účet Azure**: Potřebujete účet Microsoft Azure.
* **Úložiště Azure:** K ukládání replikovaných dat potřebujete účet úložiště Azure. Na webu Azure Portal můžete použít klasický účet nebo účet úložiště Správce prostředků. Na portálu Classic můžete použít pouze klasický účet. Replikovaná data jsou uložena v úložišti Azure a virtuální počítače Azure jsou vytvářeny, když dojde k převzetí služeb při selhání.
* **Síť Azure**: Je třeba nastavit mapování sítě, aby virtuální počítače Azure zůstaly po vytvoření a převzetí služeb při selhání připojené k příslušným sítím.
* **Server VMM**: Potřebujete jeden nebo několik místních serverů VMM běžících na System Center 2012 R2 a konfiguraci s jedním nebo několika privátními cloudy. Pokud provádíte nasazení na webu Azure Portal, potřebujete nastavit logické sítě a sítě virtuálních počítačů, aby bylo možné nakonfigurovat mapování sítě. Na portálu Classic je toto volitelné.  Síť virtuálních počítačů musí být propojena na logickou síť, která je přidružena ke cloudu.
* **Hostitel Hyper-V**: Potřebujete mít v cloudu VMM jeden nebo několik hostitelských serverů Windows Server 2012 R2 Hyper-V.
* **Virtuální počítače Hyper-V**: Potřebujete jeden nebo více virtuálních počítačů na hostitelském serveru Hyper-V.
...
- **vContinuum server (sekundární lokalita)**: Je nainstalována ve stejném umístění (sekundární lokalita) jako konfigurační server. Poskytuje konzoli pro správu a monitorování chráněného prostředí. Ve výchozí instalaci je server vContinuum prvním hlavním cílovým serverem a je na něm nainstalovaný Unified Agent.
- **Hlavní cílový server (sekundární lokalita)**: Hlavní cílový server uchovává replikovaná data. Přijímá data z procesního serveru, vytváří repliku počítače v sekundární lokalitě a ukládá body pro uchovávání dat. Počet hlavních cílových serverů, které potřebujete, závisí na počtu počítačů, které chráníte. Pokud chcete po obnovení vrátit služby do primární lokality, potřebujete hlavní cílový server i tam.
...
* **S použitím webu Azure Portal** – Site Recovery můžete nasadit přes Azure Portal. [Další informace](site-recovery-hyper-v-site-to-azure.md).
* **S použitím portálu Classic** - Službu Site Recovery můžete nasadit na portálu Classic. [Další informace](site-recovery-hyper-v-site-to-azure-classic.md).

Architektura je pro obě nasazení podobná s následující výjimkou:

- Pokud provádíte nasazení na webu Azure Portal, musíte nastavit mapování sítě. Na portálu Classic je to volitelné.
- Proces nasazení je na portálu Azure jednodušší a srozumitelnější.
...
Můžete replikovat virtuální počítače Hyper-V spravované v cloudech VMM do sekundární lokality pomocí replikace sítě SAN v portálu Azure Classic. Tento scénář není na novém portálu Azure aktuálně podporován.
...
**Účet Azure**: Potřebujete předplatné Azure.
