...
- [Ochrana počítače Azure IaaS souborového serveru](#disaster-recovery-recommendation-for-azure-iaas-virtual-machines)
- [Ochrana souborových serverů na místě](#replicate-an-onpremises-file-server-using-azure-site-recovery)
...
### <a name="factors-to-consider-while-making-disaster-recovery-decision"></a>Faktory, které je potřeba zvážit při rozhodování pro zotavení po havárii
...
**Připojení Site-to-Site**: přímé připojení mezi místními servery a síť Azure, musí vytvořit a povolit komunikaci mezi servery.  Tím lze zajistit bezpečné připojení Site-to-Site VPN do virtuální sítě v systému Windows Azure, který se použije jako lokality pro zotavení po Havárii.  
...
Při konfiguraci a správu souborových serverů, které jsou hostované na virtuální počítače Azure IaaS, můžete si vybrat mezi dvě možnosti, zotavení po havárii založené na tom, jestli chcete přesunout do [Azure Files](https://docs.microsoft.com/azure/storage/files/storage-files-introduction).
...
1.  Ochrana počítačů pomocí Azure Site Recovery pomocí kroků uvedených v tomto poli
...
6.  V případě havárie v místním prostředí, proveďte převzetí služeb při selhání pomocí plánu obnovení a přidejte skript, který chcete [připojit sdílenou složku Azure File](https://docs.microsoft.com/azure/storage/files/storage-how-to-use-files-windows) a přístup do sdílené složky ve virtuálním počítači.
...
Pokud máte místní klientům přístup k IaaS souborového serveru virtuálního počítače provést nejdřív 2 kroky taky else pokračujte krokem 3.
...
Níže kroky podrobnosti replikace pro virtuální počítače VMware, kroky pro replikaci virtuálního počítače technologie Hyper-V, najdete tady.
...
1.  Ochrana počítače serveru soubor pomocí Azure Site Recovery pomocí uvedených pokynů [sem](tutorial-vmware-to-azure.md)
...
6.  V případě havárie v místním prostředí proveďte převzetí služeb při selhání pomocí plánu obnovení a přidejte skript, který chcete připojit sdílenou složku Azure File a přístup do sdílené složky ve virtuálním počítači.
...
Další informace o provedení testu převzetí služeb při selhání, najdete v části [zde](site-recovery-failover.md).
