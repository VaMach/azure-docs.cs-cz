...
---
# <a name="step-3-plan-capacity-and-scaling-for-vmware-to-azure-replication"></a>Krok 3: Plánování kapacity a škálování Azure replikace pro VMware
...
1. Stažení [nástroj pro nasazení Planner](https://aka.ms/asr-deployment-planner) pro replikaci VMware.
2. [Přečtěte si tento článek](site-recovery-deployment-planner.md) pochopit, jak chcete nástroj spustit.
3. Pomocí nástroje shromažďovat informace o kompatibilní a nekompatibilní virtuálních počítačů, disků na virtuální počítač, a změn dat na disk. Nástroj platí i pro nároky na šířku pásma sítě a infrastrukturu Azure, které jsou potřebné pro úspěšné replikace a testovací převzetí služeb při selhání.
...
Postupujte podle těchto pokynů můžete nastavit server služby další proces. Po nastavení serveru, migrovat zdrojový počítač používat.

1. V **servery Site Recovery**, klikněte na server, konfigurace > **+ procesový Server**.
...
3. Stáhněte si soubor Unified instalace nástroje Site Recovery.
4. Spuštěním instalačního programu nainstalujte procesový server a zaregistrovat v trezoru.
5. V části **Než začnete** vyberte **Přidat další procesové servery pro horizontální navýšení kapacity nasazení**.
6. V **podrobnosti o konfiguraci serveru**, zadejte IP adresu konfiguračního serveru a heslo. Pokud nemáte heslo, získáte ho spuštěním **[SiteRecoveryInstallationFolder]\home\sysystems\bin\genpassphrase.exe – n** na konfiguračním serveru.

    ![Konfigurace serveru](./media/vmware-walkthrough-capacity/add-ps2.png)
7. Dokončete instalační program stejným způsobem, který jste to udělali při nastavení na konfiguračním serveru.

### <a name="migrate-machines-to-use-the-process-server"></a>Počítače, které chcete použít server proces migrace
