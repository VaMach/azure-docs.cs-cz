...
- [PolyBase](sql-data-warehouse-load-from-azure-blob-storage-with-polybase.md)
 ...
 Tento návod ukazuje, jak v Azure Data Factory vytvořit kanál pro přesun dat z úložiště objektů blob Azure do SQL Data Warehouse. V následujících krocích provedete následující:

+ Nastavení ukázkových dat v úložišti objektů blob Azure
...
Před zahájením tohoto kurzu musíte mít následující prostředky.
   ...
   + **Objekt blob úložiště Azure:** V tomto kurzu se používá objekt blob úložiště Azure jako zdroj dat pro kanál Azure Data Factory, takže byste měli mít nějaký k dispozici pro uložení ukázkových dat. Pokud ho ještě nemáte, přečtěte si, jak [vytvořit účet úložiště][].
   ...
   + **AZCopy**: AZCopy potřebujete ke zkopírování ukázkových dat z místního klienta do svého objektu blob úložiště Azure. Postup instalace najdete v [dokumentaci k AZCopy][].

## Krok 1: Zkopírování ukázkových dat do objektu blob úložiště Azure

Jakmile budete mít vše připraveno, můžete si zkopírovat ukázková data do objektu blob úložiště Azure.

1. [Stáhnout ukázková data][]. Tato data přidají další tři roky prodejních dat do ukázkových dat AdventureWorksDW.

2. Pomocí tohoto příkazu AZCopy si můžete do objektu blob úložiště Azure zkopírovat tři roky dat.
...
Začněte tím, že otevřete [portál Azure][] a z nabídky na levé straně vyberete datovou továrnu.

### Krok 2.1: Vytvoření propojené služby

Propojte si účet úložiště Azure a SQL Data Warehouse se svojí datovou továrnou.  

1. Začněte proces registrace kliknutím na část Propojené služby svojí datové továrny a potom klikněte na Nové datové úložiště. Zvolte název, pod kterým si chcete svoje úložiště Azure zaregistrovat, jako typ vyberte Úložiště Azure a pak zadejte název a klíč účtu.
...
Po vytvoření propojených služeb budeme muset definovat datové sady.  Zde to znamená definovat strukturu dat, která se přesouvá z vašeho úložiště do vašeho datového skladu.  O vytváření si toho můžete přečíst víc.

1. Tento proces spusťte tak, že ve své datové továrně přejdete do části Vytvořit a nasadit.
...
## Krok 3: Vytvoření a spuštění vašeho kanálu

Nakonec v Azure Data Factory nastavíme a spustíme kanál.  Je to operace, která se provede až po tom, co se data skutečně přesunou.  Úplný přehled operací, které můžete s SQL Data Warehouse a Azure Data Factory provádět, najdete [tady][Přesun dat do a z Azure SQL Data Warehouse pomocí Azure Data Factory].

V části Vytvořit a nasadit teď klikněte na Další příkazy a pak klikněte na Nový kanál.  Po vytvoření kanálu si můžete pomocí níže uvedeného kódu přenést data do datového skladu:
...
Pokud se chcete dozvědět více, začněte těmito zdroji informací:
...
- [Kurz: Začínáme s Azure Data Factory:][] Jde o základní kurz pro zpracování dat pomocí služby Azure Data Factory. V tomto kurzu si vytvoříte svůj první kanál, který využívá HDInsight k transformaci a měsíční analýze webových protokolů. Poznámka: Tento kurz neobsahuje žádné aktivity kopírování.
...
- [Kurz se scénářem z reálného prostředí:][] Toto je podrobný kurz používání služby Azure Data Factory.
...
[Načtení ukázkových dat do SQL Data Warehouse]: sql-data-warehouse-get-started-manually-load-samples.md
...
[Kurz se scénářem z reálného prostředí:]: ../data-factory/data-factory-tutorial.md
[Kurz: Kopírování dat z Azure Blob Storage do Azure SQL Database]: ../data-factory/data-factory-get-started
[Kurz: Začínáme s Azure Data Factory:]: ../data-factory/data-factory-build-your-first-pipeline.md

<!--MSDN references-->

<!--Other Web references-->
[Přehled Azure Data Factory]: https://azure.microsoft.com/documentation/learning-paths/data-factory
