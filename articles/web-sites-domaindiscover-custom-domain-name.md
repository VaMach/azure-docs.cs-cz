...
5. On the following page, enter the DNS record values. Then click the **Add** button.
    ...
    * When adding a CNAME record, you must first select **CNAME (Alias)** on the **Manage DNS** page. Then set the **Host** field to the sub-domain you wish to use. For example, **www**. You must set the **Alias Hostname** field to the **.chinacloudsites.cn** domain name of your Azure Web Site. For example, **contoso.chinacloudsites.cn**. Then provide a Time-to-Live (TTL) value, such as 1800 seconds.
    ...
    * When adding an A record, you must first select **A** on the **Manage DNS** page. Then set the **Host** field to either **@** (this represents root domain name, such as **contoso.com**,) or the sub-domain you wish to use (for example, **www**.) You must set the **IP Address** field to the IP address of your Azure Web Site. Then provide a Time-to-Live (TTL) value such as 1800 seconds.
		...
		> 
		> * A **Host** value of **www** with an **Alias Hostname** value of **&lt;yourwebsitename&gt;.chinacloudsites.cn**.
		> 
		> OR
		> 
		> * A **Host** value of **awverify.www** with an **Alias Hostname** value of **awverify.&lt;yourwebsitename&gt;.chinacloudsites.cn**.
		> 
