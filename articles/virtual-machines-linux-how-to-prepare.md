...
7.  CentOS 6.2 or 6.3: Install the drivers for the Linux Integration Services
    ...
    **Note:** The step is only valid for CentOS 6.2 and 6.3. In CentOS 6.4+ the Linux Integration Services are already available in the kernel.
    ...
    ![Add DVD drive with install media][]
...
**NOTE:** [SUSE Studio][] can easily create and manage your SLES/opeSUSE images for Azure and Hyper-V. In addition, the following official images in the SUSE Studio Gallery can be downloaded or cloned into your own SUSE Studio account for easy customization:
...
3.  Add the repository containing the latest kernel and Azure Linux Agent. Run the command `zypper lr`. For example, with SLES 11 SP3 the output should look similar to the following:
    ...
    then the repositories may need to be re-enabled or the system registered. This can be done via the suse\_register utility. For more information, please see the [SLES documentation][].
...
3.  Modify the kernel boot line in Grub to include the following parameters. This will also ensure all console messages are sent to the first serial port, which can assist Azure support with debugging issues:

        console=ttyS0 earlyprintk=ttyS0 rootdelay=300

4.  It is recommended that you set /etc/sysconfig/network/dhcp or equivalent from DHCLIENT\_SET\_HOSTNAME="yes" to DHCLIENT\_SET\_HOSTNAME="no"

5.  In /etc/sudoers, comment out the following line, if it exists:
...
9.  Click **Shutdown** in Hyper-V Manager.
...
1.  Disable automatic DVD ROM probing.
...
10. Click **Shutdown** in Hyper-V Manager.
...
## <span id="#connect"></span> </a>Step 3: Prepare the connection to Azure
...
## <span id="upload"></span> </a>Step 4: Upload the image to Azure
...
It is for this reason that we recommend that you start with one of our [partners endorsed images][].

The list below replaces step 1 of the process to create your own VHD:

1.  You will need to ensure that you are running a kernel that either incorporates the latest LIS drivers for Hyper-V or that you have successfully compiled them ( They have been Open Sourced). The Drivers can be found [at this location][]

2.  Your kernel should also include the latest version of the ATA PiiX driver that is used to to provision the iamges and has the fixes committed to the kernel with commit cd006086fa5d91414d8ff9ff2b78fbb593878e3c Date: Fri May 4 22:15:11 2012 +0100 ata\_piix: defer disks to the Hyper-V drivers by default
...
5.  It is recommended that you set /etc/sysconfig/network/dhcp or equivalent from DHCLIENT\_SET\_HOSTNAME="yes" to DHCLIENT\_SET\_HOSTNAME="no"

6.  You should Ensure that all SCSI devices mounted in your kernel include an I/O timeout of 300 seconds or more.

7.  You will need to install the Azure Linux Agent following the steps in the [Linux Agent Guide][]. The agent has been released under the Apache 2 license and you can get the latest bits at the [Agent GitHub Location][]
...
12. You will then need to shut down the virtual machine and proceed with the upload.
  ...
  [Manage Disks and Images]: http://msdn.microsoft.com/en-us/library/windowsazure/jj672979.aspx
  [How to Create a Custom Virtual Machine]: /en-us/manage/windows/how-to-guides/custom-create-a-vm/
  [this article]: http://support.microsoft.com/kb/2805216
  [Create a Management Certificate for Azure]: http://msdn.microsoft.com/en-us/library/windowsazure/gg551722.aspx
  ...
  [Azure Command-Line Tools for Linux and Mac]: http://go.microsoft.com/fwlink/?LinkID=253691&clcid=0x409
  [Azure Downloads]: /en-us/develop/downloads/
  [Add-AzureVhd]: http://msdn.microsoft.com/en-us/library/windowsazure/dn205185.aspx
  [Microsoft web site]: http://go.microsoft.com/fwlink/?LinkID=253692&clcid=0x409
  [Step 1: Prepare the image to be uploaded]: #prepimage
  [Step 2: Create a storage account in Azure]: #createstorage
  [Step 3: Prepare the connection to Azure]: #connect
  [Step 4: Upload the image to Azure]: #upload
  [Download Center]: http://www.microsoft.com/en-us/download/details.aspx?id=34603
  ...
  [SLES documentation]: https://www.suse.com/documentation/sles11/
  ...
  [Get Started with Azure Cmdlets]: http://msdn.microsoft.com/en-us/library/windowsazure/jj554332.aspx
  [partners endorsed images]: https://www.windowsazure.com/en-us/manage/linux/other-resources/endorsed-distributions/
  [at this location]: http://go.microsoft.com/fwlink/p/?LinkID=254263&clcid=0x409
  [Linux Agent Guide]: https://www.windowsazure.com/en-us/manage/linux/how-to-guides/linux-agent-guide/
