...
* [**Elastické škálování propustnosti a úložiště na vyžádání, celosvětově**](#horizontal-scale)
    * Snadno škálujte propustnost v přírůstcích po [sekundách](request-units.md) a [minutách](https://aka.ms/acdbrupm), přičemž to můžete kdykoli změnit. 
...
* [**Zápis globálně distribuovaných aplikací správným způsobem**](#consistency)
    * [Pět modelů konzistence](consistency-levels.md) nabízí silnou konzistenci pravděpodobnosti SQL až pravděpodobnosti NoSQL a cokoli mezi tím. 
...
Globální distribuce prostředků v rámci služby Cosmos DB je [na klíč](distribute-data-globally.md). Se svým databázovým účtem můžete kdykoli propojit libovolný počet zeměpisných oblastí. Stačí k tomu kliknout na pár tlačítek (nebo programově volat rozhraní API). 
...
## Podpora více modelů, více API
 ...
 Služba také podporuje známá databázová rozhraní API pro přístup k datům a dotazování na ně. Databázový stroj Cosmos DB v současné době podporuje rozhraní API [DocumentDB SQL](documentdb-introduction.md), [MongoDB](mongodb-introduction.md), [Azure Tables](table-introduction.md) (Preview) a [Gremlin](graph-introduction.md) (Preview). Můžete dále sestavovat aplikace pomocí známých rozhraní API OSS a získat všechny výhody v praxi testované a plně spravované globálně distribuované databázové služby. 

<a id="horizontal-scale"></a>
<a id="horizontal-scaling-of-storage-and-throughput" class="xliff"></a>

## Horizontální škálování úložiště a propustnosti
...
Propustnost kontejneru Azure Cosmos DB můžete elasticky škálovat programovým zřizováním propustnosti pomocí [jednotek požadavků za sekundu (RU/s)](request-units.md). Interně služba transparentně spravuje oddíly prostředků pro zajištění propustnosti na daném kontejneru. Cosmos DB zajišťuje, že propustnost je k dispozici napříč všemi oblastmi spojenými s daným kontejnerem. Nová propustnost začíná platit do pěti sekund od změny konfigurované hodnoty propustnosti. 
...
## Záruky nízké latence na 99. percentilu
...
## Transparentní souběžná instalace a 99,99% vysoká dostupnost
...
## Více řádně definovaných modelů konzistence
...
Cosmos DB umožňuje volbu mezi [pěti správně definovanými modely konzistence](consistency-levels.md) podél spektra konzistence – silná, omezená neaktuálnost, [relace](http://dl.acm.org/citation.cfm?id=383631), předpona konzistence a nahodilé. 
...
## Zaručené smlouvy o úrovni služeb
