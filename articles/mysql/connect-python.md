...
Nainstalujte na svém počítači [Python](https://www.python.org/downloads/) a [konektor MySQL pro Python](https://dev.mysql.com/downloads/connector/python/). V závislosti na vaší platformě postupujte podle příslušných kroků:
...
- Pokud chcete kód spustit, spusťte příkazový řádek nebo prostředí Bash. Změňte adresář na složku vašeho projektu pomocí příkazu `cd pythonmysql`. Zadejte příkaz python následovaný názvem souboru (`python createtable.py`) a spusťte aplikaci. Pokud se v operačním systému Windows nepodaří najít soubor python.exe, můžete zadat úplnou cestu ke spustitelnému souboru nebo přidat cestu k Pythonu do proměnné prostředí PATH. `C:\python27\python.exe createtable.py`
