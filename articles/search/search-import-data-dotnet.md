<properties
    pageTitle="Odesílání dat do služby Azure Search pomocí .NET SDK | Microsoft Azure | Hostovaná cloudová vyhledávací služba"
...
> [AZURE.NOTE] V této ukázce používáme různé akce na různé dokumenty. Místo volání `IndexBatch.New` můžete použít jiné statické metody třídy `IndexBatch`, pokud chcete provádět stejné akce na všech dokumentech v dávce. Dávky můžete vytvořit například voláním `IndexBatch.Merge`, `IndexBatch.MergeOrUpload` nebo `IndexBatch.Delete`. Tyto metody přijímají místo objektů `IndexAction` kolekci dokumentů (v této ukázce objekty typu `Hotel`).
