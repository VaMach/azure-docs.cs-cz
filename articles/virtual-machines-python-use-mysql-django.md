...
1.  Follow the instructions given [here][] to create an Azure virtual machine of the *Windows Server 2008 R2* distribution.

2.  Open up a TCP port for MySQL transactions on the virtual machine:
...
2.  Use Windows *Remote Desktop* to remotely log into the newly created Azure virtual machine.

3.  Open up TCP port **80** on the virtual machine:
...
1.  Install the latest version of [MySQL Community Server][] for Windows on the virtual machine:

    **Note**: we recommend installing your DB on a different data partition than the OS.

-   Run the *Windows (x86, 64-bit), MSI Installer* link [here][MySQL Community Server] and download the appropriate MSI installer from the download mirror nearest you. Helpful hints are:
...
-   Install a sample ["world" database][] (MyISAM version):

    -   Download [this][] zip file on the Azure Virtual Machine.
...
1.  Before you can begin developing your Django application, you of course need to install Python+Django on the virtual machine. You can do so by using the [Web Platform Installer][]. After installing Web PI, use it to search for "Django" and install the Django (Python) product.
...
1.  Install the MySQL Python client package. You can install it directly [from this link][]. Once completed, run the following command to verify your installation:

![][2]

## Extend the Django Hello World application

1.  Follow the instructions given in the [Django Hello World][] tutorial to create a trivial "Hello World" web application in Django.

2.  Open **C:\\django\\helloworld\\helloworld\\settings.py** in your favorite text editor. Modify the **DATABASES** global dictionary to read:
...
-   Edit **C:\\django\\helloworld\\helloworld\\settings.py** again. Add *'counter'* to the *INSTALLED\_APPS* tuple.
...
2.  From your local web browser, open \*\*<http://*yourVmName>\*.cloudapp.net\*\* (where *yourVmName* is whatever name you used in the virtual machine creation step). You should see "Hello ... !" displayed as shown in the screenshot below. This indicates that Django is running in the virtual machine and is working correctly.
...
When you're done with this tutorial, shutdown and/or remove your newly created Azure virtual machine to free up resources for other tutorials and avoid incurring Azure usage charges.
  ...
  [MySQL web site]: http://dev.mysql.com/doc/
  [MySQL driver]: http://pypi.python.org/pypi/MySQL-python/1.2.3
  []: ./media/virtual-machines-python-use-mysql-django/mysql_tutorial01.png
  [create-account-and-vms-note]: ../includes/create-account-and-vms-note.md
  [here]: /en-us/manage/windows/tutorials/virtual-machine-from-gallery/
  [MySQL Community Server]: http://dev.mysql.com/downloads/mysql/
  ["world" database]: http://dev.mysql.com/doc/index-other.html
  [this]: http://downloads.mysql.com/docs/world.sql.zip
  [1]: ./media/virtual-machines-python-use-mysql-django/mysql_tutorial01-2.png
  [Web Platform Installer]: http://www.microsoft.com/web/downloads/platform.aspx
  [from this link]: http://code.google.com/p/soemin/downloads/detail?name=MySQL-python-1.2.3.win32-py2.7.exe&can=2&q=
