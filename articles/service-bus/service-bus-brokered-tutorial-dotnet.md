...
1. Pokud chcete vytvořit obor názvů služby, přejděte na [klasický portál Azure][]. V levé čísti klikněte na **Service Bus** a pak na **Vytvořit**. Zadejte název oboru názvů a pak klikněte na zatržítko.

1. V hlavním okně portálu klikněte na název oboru názvů, který jste vytvořili v předchozím kroku.

1. Klikněte na **Konfigurovat**.

1. Poznamenejte si primární klíč přiřazený k zásadám **RootManagerSharedAccessKey** nebo ho zkopírujte do schránky. Tuto hodnotu použijete později v tomto kurzu.
...
1. Vytvořte nový projekt konzolové aplikace. Klikněte na nabídku **Soubor** a vyberte možnost **Nový**, a pak klikněte na **Projekt**. V dialogu **Nový projekt** klikněte na **Visual C#** (pokud se **Visual C#** nezobrazí, podívejte se do části **Jiné jazyky**), klikněte na šablonu **Konzolová aplikace** a pojmenujte ji jako **QueueSample**. Použijte výchozí **Umístění**. Klikněte na **OK**, tím vytvoříte projekt.

1. Pomocí správce balíčků NuGet do projektu přidejte knihovny ServiceBus:
...
<br />
1. V Průzkumníku řešení poklikejte na soubor Program.cs a otevře se v editoru Visual Studio. Změňte název oboru názvů z výchozího názvu `QueueSample` na `Microsoft.ServiceBus.Samples`.
...
1. Upravte příkazy `using` tak, jak je vidět na následujícím kódu.
...
1. Vytvořte textový soubor s názvem Data.csv a zkopírujte do něj následující text oddělený čárkami.
...
1. V Průzkumníku řešení klikněte pravým tlačítkem na název projektu (v tomto příkladu **QueueSample**), klikněte na **Přidat**, a pak klikněte na **Existující položka**.
...
1. Mimo `Main()` definujte metodu `ParseCSV()`, která parsuje seznam zpráv v souboru Data.csv a načte zprávy do tabulky [DataTable](https://msdn.microsoft.com/library/azure/system.data.datatable.aspx), jak je vidět tady. Tato metoda vrátí objekt **DataTable**.
...
1. V metodě `Main()` přidejte příkaz, který zavolá metodu `ParseCSVFile()`:
...
1. Do `Main()` přímo po zavolání `ParseCSVFile()` přidejte příkaz, který zavolá metodu `GenerateMessages()` s hodnotou vrácenou z `ParseCSVFile()` jako argument:
...
1. Dál vytvořte funkci, která přijme a uloží obor názvů služby a klíč SAS. Tuto metodu přidejte mimo `Main()`. Příklad: 
...
1. Do `Main()` přímo po zavolání `GenerateMessages()` přidejte příkaz, který zavolá metodu `CollectUserInput()`:
...
1. Dalším krokem je vytvoření pověření SAS pomocí objektu [TokenProvider](https://msdn.microsoft.com/library/azure/microsoft.servicebus.tokenprovider.aspx). Metoda vytvoření převezme název a hodnotu klíče SAS získané v metodě `CollectUserInput()`. Do metody `Queue()` přidejte následující kód:
...
2. Vytvořte nový objekt správy oboru názvů s identifikátorem URI, který jako argumenty obsahuje název a pověření pro správu získané v předchozím kroku. Tento kód přidejte přímo po kódu přidaném v předchozím kroku. Ujistěte se, že jste `<yourNamespace>` nahradili názvem vašeho oboru názvů služby:
...
1. V metodě `Queue()` vytvořte objekt pro vytváření pro přenos zpráv s nově vytvořeným identifikátorem URI služby Service Bus jako argument. Následující kód přidejte přímo po operacích správy, které jste přidali v předchozím kroku. Ujistěte se, že jste `<yourNamespace>` nahradili názvem vašeho oboru názvů služby:

    ```
    MessagingFactory factory = MessagingFactory.Create(ServiceBusEnvironment.CreateServiceUri("sb", "<yourNamespace>", string.Empty), credentials);
    ```

1. Dál vytvořte objekt fronty pomocí třídy [QueueClient](https://msdn.microsoft.com/library/azure/microsoft.servicebus.messaging.queueclient.aspx). Následující kód přidejte přímo po kódu, který jste přidali v posledním kroku:

    ```
    QueueClient myQueueClient = factory.CreateQueueClient("IssueTrackingQueue");
    ```

1. Dál přidejte kód který projde seznam zprostředkovaných zpráv, které jste předtím vytvořili, a každou z nich odešle do fronty. Následující kód přidejte přímo po příkazu `CreateQueueClient()` v předchozím kroku:
