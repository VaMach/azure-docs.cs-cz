<properties 
    pageTitle="Kurz: Vytvoření kanálu s aktivitou kopírování pomocí Azure PowerShellu" 
...
Pokud používáte prostředí Azure PowerShell **verze nižší než 1.0**, budete muset použít rutiny popsané [tady][old-cmdlet-reference]. Než začnete používat rutiny služby Data Factory, budete také muset spustit následující příkazy: 
...
1. Otevřete prostředí Azure PowerShell a spusťte následující příkazy. Nechte prostředí Azure PowerShell otevřené až do konce tohoto kurzu. Pokud ho zavřete a znovu otevřete, tyto příkazy bude potřeba znovu spustit.
    ...
    3. Spuštěním příkazu **Select-AzureSubscription** vyberte předplatné, se kterým chcete pracovat. Mělo by to být stejné předplatné, které jste použili na webu Azure Portal.
4. Přepněte do režimu AzureResourceManager, v něm jsou totiž dostupné rutiny služby Azure Data Factory: **Switch-AzureMode AzureResourceManager**.
...
[Vytvoření vstupní a výstupní datové sady](#create-datasets) | V tomto kroku nadefinujete dvě datové sady, **EmpTableFromBlob** a **EmpSQLTable**, které se používají jako vstupní a výstupní tabulka pro **aktivitu kopírování** v objektu ADFTutorialPipeline, který vytvoříte v následujícím kroku.
...
1. Otevřete prostředí Azure PowerShell a spusťte následující příkaz. Nechte prostředí Azure PowerShell otevřené až do konce tohoto kurzu. Pokud ho zavřete a znovu otevřete, tyto příkazy bude potřeba znovu spustit.
    ...
    - Spuštěním příkazu **Get-AzureSubscription** zobrazte všechna předplatná pro tento účet.
    - Spuštěním příkazu **Select-AzureSubscription <Name of the subscription>** vyberte předplatné, se kterým chcete pracovat. Mělo by to být stejné předplatné, které jste použili na webu Azure Portal.
3. Spuštěním následujícího příkazu vytvořte skupinu prostředků Azure s názvem **ADFTutorialResourceGroup**.
   
        New-AzureRmResourceGroup -Name ADFTutorialResourceGroup  -Location "West US"

    Některé kroky v tomto kurzu vychází z předpokladu, že používáte skupinu prostředků s názvem **ADFTutorialResourceGroup**. Pokud máte jinou skupinu prostředků, používejte ji v postupech v tomto kurzu místo skupiny ADFTutorialResourceGroup. 
...
1.  Ve složce **C:\ADFGetStartedPSH** vytvořte soubor JSON s názvem **StorageLinkedService.json** s následujícím obsahem. Pokud složka ADFGetStartedPSH neexistuje, vytvořte ji.
    ...
    Položku **accountname** nahraďte názvem účtu úložiště a **accountkey** nahraďte klíčem pro účet úložiště Azure.
2.  V prostředí **Azure PowerShell** přejděte do složky **ADFGetStartedPSH**. 
3.  K vytvoření propojené služby můžete použít rutinu **New-AzureRmDataFactoryLinkedService**. Tato rutina a jiné rutiny služby Data Factory používané v tomto kurzu vyžadují, abyste zadali hodnoty parametrů **ResourceGroupName** a **DataFactoryName**. Alternativně můžete pomocí rutiny **Get-AzureRmDataFactory** načíst objekt DataFactory a tento objekt předat, abyste nemuseli při každém spouštění rutiny zadávat hodnoty parametrů ResourceGroupName a DataFactoryName. Spuštěním následujícího příkazu přiřaďte výstup rutiny **Get-AzureRmDataFactory** k proměnné **$df**. 
...
Tabulka je obdélníková datová sada a je jediným typem datové sady, který je v současné době podporovaný. Vstupní tabulky v tomto kurzu odkazuje na kontejner objektů blob ve službě Azure Storage, na kterou odkazuje objekt StorageLinkedService, a výstupní tabulka odkazuje na tabulku SQL ve službě Azure SQL Database, na kterou odkazuje objekt AzureSqlLinkedService.  

### Příprava služeb Azure Blob Storage a Azure SQL Database pro tento kurz
Tento krok přeskočte, pokud jste prošli kurzem z článku [Začínáme s Azure Data Factory][adf-get-started]. 
...
3. Pomocí následujícího skriptu SQL vytvořte tabulku **emp** v Azure SQL Database.  
    ...
    Pokud máte na počítači nainstalovanou sadu Visual Studio 2013: Na webu Azure Portal ([http://portal.azure.com](http://portal.sazure.com)), klikněte na **PROCHÁZET** na levé straně, klikněte na **SQL**, vyberte svou databázi a potom se kliknutím na tlačítko **Otevřít v sadě Visual Studio** na panelu nástrojů připojte k serveru SQL Azure a spusťte skript. Pokud klient nemá povolený přístup k serveru SQL Azure, budete muset nakonfigurovat bránu firewall pro server SQL Azure tak, aby povolovala přístup z vašeho počítače (IP adresa). Postup konfigurace brány firewall pro server SQL Azure najdete v článku výš.
...
V této části kroku vytvoříte výstupní tabulku s názvem **EmpSQLTable**, která odkazuje na tabulku SQL (**emp**) ve službě Azure SQL Database reprezentované propojenou službou **AzureSqlLinkedService**. Kanál kopíruje data z vstupního objektu blob do tabulky **emp**. 
...
1.  Ve složce **C:\ADFGetStartedPSH** vytvořte soubor JSON s názvem **EmpSQLTable.json** s následujícím obsahem:
    ...
    * Vlastnost **availability** je nastavená na **hourly** (**frequency** je nastavená na **hour** a **interval** je nastavená na **1**).  Služba Data Factory bude generovat řez výstupních dat každou hodinu do tabulky **emp** ve službě Azure SQL Database.
...
1.  Ve složce **C:\ADFGetStartedPSH** vytvořte soubor JSON s názvem **ADFTutorialPipeline.json** s následujícím obsahem: 
    ...
    V uvedeném příkladu bude k dispozici 24 datových řezů, protože se vytvářejí každou hodinu.
...
2.  Spuštěním rutiny **Get-AzureRmDataFactorySlice** získejte podrobnosti o všech řezech tabulky **EmpSQLTable**, která je výstupní tabulkou kanálu.  
    ...
    Rok, měsíc a den v parametru **StartDateTime** nahraďte aktuálním rokem, měsícem a dnem. Mělo by to odpovídat hodnotě **Start** v kódu JSON kanálu. 
...
3.  Spuštěním rutiny **Get-AzureRmDataFactoryRun** získáte podrobnosti o spouštění aktivity pro **konkrétní** řez. Změňte hodnotu parametru **StartDateTime** tak, aby odpovídala času **Start** řezu ve výstupu výš. Hodnota **StartDateTime** musí být ve [formátu ISO](http://en.wikipedia.org/wiki/ISO_8601). Příklad: 2014-03-03T22:00:00Z.
...
## Souhrn
V tomto kurzu jste vytvořili objekt pro vytváření dat Azure pro zkopírování dat z objektu blob Azure do Azure SQL Database. PowerShell jste použili k vytvoření objektu pro vytváření dat, propojených služeb, datových sad a kanálu. Zde jsou základní kroky, které jste provedli v tomto kurzu:  
