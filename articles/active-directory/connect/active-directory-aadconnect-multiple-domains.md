...
Pomocí příkazu prostředí PowerShell můžete zobrazení IssuerUri `Get-MsolDomainFederationSettings -DomainName <your domain>`.
...
[!NOTE]
Poslední číslo v regulárním výrazu nastavit kolik nadřazené domény v kořenové doméně. I tady mají bmcontoso.com, takže jsou potřebné dvě nadřazené domény. V případě, že tři nadřazené domény byly budou muset zůstat (tj.: corp.bmcontoso.com), pak číslo by byl tři. Eventualy rozsah můžete zadat shody bude vždy provést tak, aby odpovídala maximální počet domén. "{2,3}" bude odpovídat dvě až tři domény (např: bmfabrikam.com a corp.bmcontoso.com).
