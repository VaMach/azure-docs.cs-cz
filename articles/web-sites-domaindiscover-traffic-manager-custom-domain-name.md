...
5. On the following page, enter the DNS record values. Then click the **Add** button.
    ...
    * When adding a CNAME record, you must first select **CNAME (Alias)** on the **Manage DNS** page. Then set the **Host** field to the sub-domain you wish to use. For example, **www**. You must set the **Alias Hostname** field to the **.trafficmanager.cn** domain name of the Traffic Manager profile you are using with your Azure Web Site. For example, **contoso.trafficmanager.cn**. Then provide a Time-to-Live (TTL) value, such as 1800 seconds.
