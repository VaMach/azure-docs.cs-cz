<properties
    pageTitle="Přidání přihlášení k Azure B2C ve verzi Preview do webové aplikace Node.js | Microsoft Azure"
...
# B2C ve verzi Preview: Přidání přihlašování do webové aplikace Node.js
...
> [AZURE.WARNING]   Pro tento B2C ve verzi Preview musíte použít stejné **ID klienta**/**ID aplikace** a zásady jak pro server, starající se o úkoly webového rozhraní API, tak pro klienta, který se k němu připojuje. To samé platí pro kurzy o iOS a Androidu. Pokud jste již dříve vytvořili aplikaci v některém z těchto rychlých průvodců, použijte tyto hodnoty, nevytvářejte nové.
...
- Do pole **Adresa URL odpovědi** vyplňte `http://localhost/TodoListService`. To je výchozí URL pro tento příklad.
...
## Přidání požadovaných součástí do adresáře
...
## Nastavení aplikace pro použití strategie Passport-Node.js
...
## Použití Passportu pro zasílání požadavků na přihlášení a odhlášení do Azure AD

Vaše aplikace je nyní správně nastavená pro komunikaci s koncovým bodem v2.0 pomocí ověřovacího protokolu OpenID Connect. `passport-azure-ad` už se postaral o podrobnosti ohledně vytváření ověřovacích zpráv, ověřování tokenů z Azure AD a udržování uživatelských relací. Už jen zbývá umožnit uživatelům přihlášení a odhlášení a sbírat dodatečné informace o přihlášených uživatelích.
...
## Vytvoření zobrazení a tras v Expressu pro volání zásad
