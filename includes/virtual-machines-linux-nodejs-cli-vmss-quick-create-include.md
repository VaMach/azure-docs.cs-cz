...
Pokud chcete přizpůsobit umístění nebo IMAGE-URN, podívejte se na příkazy `azure location list` a `azure vm image {list-publishers|list-offers|list-skus|list|show}`.

Tento příkaz vrátí řízení, jakmile se nasazení škálovací sady dokončí. V této chvíli bude tato škálovací sada obsahovat nástroj pro vyrovnávání zatížení s pravidly překladu adres (NAT) mapujícími port 50 000+i v tomto nástroji pro vyrovnávání zatížení na port 22 ve virtuálním počítači s označením i. Díky tomu se po zjištění plně kvalifikovaného názvu domény nástroje pro vyrovnávání zatížení můžeme připojit k virtuálním počítačům přes SSH:
