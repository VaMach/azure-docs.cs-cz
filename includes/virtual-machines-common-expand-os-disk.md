...
Při vytváření nového virtuálního počítače ve skupině prostředků nasazením image z webu [Azure Marketplace](https://azure.microsoft.com/marketplace/) je výchozí velikost jednotky operačního systému 127 GB. I když je možné k virtuálnímu počítači přidat datové disky (počet závisí na zvolené skladové položce) a navíc se na tyto dodatečné disky doporučuje instalovat aplikace a úlohy náročné na procesor, zákazníci často potřebují rozšířit jednotku operačního systému pro zajištění podpory určitých scénářů, jako jsou například tyto:
...
5. A tady přichází moment, na který jsme čekali! Nastavte velikost disku operačního systému na požadovanou hodnotu a aktualizujte virtuální počítač následujícím způsobem:
   ...
   > [!WARNING]
   > Nová velikost musí být větší než stávající velikost disku. Maximální povolená velikost je 1 023 GB.
   > 
   > 
